 ...
 */
export const fetchEventRows = async (query: DashboardQuery): Promise<MonitorEventRow[]> => {
  const client = useSupabase() as any;
  let req = client
    .from("monitor_events")
  ...
  if (query.appId) req = req.eq("app_id", query.appId);
  if (query.env) req = req.eq("env", query.env);

  const { data, error } = await req;
  if (error) throw error;
  return (data ?? []) as MonitorEventRow[];
};
...
export const fetchRecentErrors = async (
  ...
  const { data, error } = await req;
  if (error) throw error;
  return (data ?? []) as MonitorErrorRow[];
};
