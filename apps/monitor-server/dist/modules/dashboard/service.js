...
const toResourceSamples = (value) => {
    ...
    value.forEach((item) => {
        ...
        const rawInitiatorType = typeof entry.initiatorType === "string" ? entry.initiatorType.trim() : "";
 ...
 * 根据查询区间选择窗口粒度（小时/天）。
 */
const resolveWindowGranularity = (from, to) => {
 ...
 * 采用倒序扫描以优先命中近期数据。
 */
const getWindowIndex = (windows, timestampMs) => {
    for (let i = windows.length - 1; i >= 0; i -= 1) {
        const window = windows[i];
        if (!window)
            continue;
        if (timestampMs >= window.start)
            return i;
    }
    return -1;
};
 ...
 * 按查询区间创建空的窗口聚合容器。
 */
const buildEmptyStats = (query) => buildWindows(query.from, query.to).map((w) => ({
    label: w.label,
    requestCount: 0,
    errorCount: 0,
    durations: [],
 ...
 */
const aggregateRows = (rows, query) => {
    const windows = buildWindows(query.from, query.to);
    const stats = buildEmptyStats(query);
    const globalDurations = [];
    const routeCounter = { page: {}, api: {} };
    const routeDurationCounter = {
        api: {},
        request: {},
    };
    const resourceDurationCounter = {};
    for (const row of rows) {
        ...
        if (row.event_name === "page-view") {
            const path = extractPath(props.url);
            if (path)
                routeCounter.page[path] = (routeCounter.page[path] ?? 0) + 1;
            continue;
        }
        if (row.event_name === "performance-paint") {
            ...
            if (fcpMs !== null && fcpMs >= 0)
                bucket.fcpValues.push(fcpMs / 1000);
            continue;
        }
        ...
        if (row.event_name === "performance-listening") {
            ...
            if (lcpMs !== null && lcpMs >= 0)
                bucket.lcpValues.push(lcpMs / 1000);
            if (cls !== null && cls >= 0)
                bucket.clsValues.push(cls);
        }
    }
 ...
 * 百分比以 Top-N 子集总和为分母。
 */
const toRouteMetrics = (counter, limit) => {
 ...
 */
const toSlowRouteMetrics = (counter, limit) => Object.entries(counter)
    .map(([path, stats]) => ({
    path,
    avgDurationMs: Number(quantile([...stats.durations].sort((a, b) => a - b), 0.95).toFixed(2)),
 ...
 */
const toSlowResourceMetrics = (counter, limit) => Object.values(counter)
    .map((stats) => ({
    name: stats.name,
    ...
    .sort((a, b) => b.avgDurationMs - a.avgDurationMs)
    .slice(0, limit);
const buildDashboardKpis = (stats, globalDurations) => {
...
const calcTrendPercent = (current, previous, precision = 2) => {
...
const buildKpiTrends = (current, previous) => ({
...
export const getDashboardOverview = async (query) => {
    ...
    const { stats, globalDurations, routeCounter, routeDurationCounter, resourceDurationCounter } = aggregateRows(events, query);
    ...
    const recentErrors = errors.map((item, idx) => ({
...
export const getDashboardCharts = async (query) => {
    ...
    const requestErrorTrend = stats.map((s) => ({
        timeWindow: s.label,
        requests: s.requestCount,
        errorRate: s.requestCount ? Number((s.errorCount / s.requestCount).toFixed(4)) : 0,
    }));
    const latencyPercentileTrend = stats.map((s) => {
    ...
    const resourcePercentileTrend = stats.map((s) => {
 ...
 */
export const getDashboardRoutes = async (query, type = "api", limit = 10) => {
    const events = await fetchEventRows(query);
 ...
 */
export const getDashboardErrors = async (query, limit = 20) => {
